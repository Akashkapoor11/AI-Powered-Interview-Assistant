...
export function generateQuestions(profile?: any): Question[] {
  ...
  const hard: Question[] = [
    {
      id: "q5",
      text: "Design an SSR strategy for a React app that fetches data and hydrates on the client. What pitfalls would you watch for?",
      difficulty: "HARD",
      seconds: 120,
    },
    {
      id: "q6",
      text: "Given a high-traffic Node.js service, how would you profile, detect bottlenecks (CPU/I/O), and scale horizontally while keeping sessions consistent?",
      difficulty: "HARD",
      seconds: 120,
    },
  ];
...
function offlineHeuristicScore(q: Question, answer: string): Scored {
  ...
  const verdict: Scored["verdict"] = pts >= max * 0.8 ? "correct" : pts >= max * 0.35 ? "partially_correct" : "incorrect";
  return {
    points: pts,
    verdict,
    feedback:
  ...
  for (const model of GEMINI_MODELS) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.warn(`[Gemini] HTTP ${res.status} on ${model}. Falling back.`, body.slice(0, 120));
        return null;
      }
      ...
      else console.warn(`[Gemini] ${model} network error.`, String(err).slice(0, 100));
      continue;
    }
  }
  return null;
...
export async function scoreAnswer(q: Question, answer: string): Promise<Scored> {
  if (!answer || !answer.trim()) return { points: 0, verdict: "incorrect", feedback: "No answer provided." };
  ...
  try {
    ...
    const feedback = parsed.feedback?.trim() || "No feedback provided";
...
export async function summarizeCandidate(profile: any, answers: Array<{ q: Question; answer: string; score: number; verdict?: string }>, finalScore: number): Promise<string> {
  if (!GEMINI_KEY) {
  ...
  const prompt = [
    ...
    `Results:\n${short}`,
    `Write 2–3 sentences about strengths, weaknesses, and recommendation.`,
  ].join("\n");
